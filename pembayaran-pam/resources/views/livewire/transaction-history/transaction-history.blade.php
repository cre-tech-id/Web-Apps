        ...
        <div class="dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-left">
            ...
            @foreach ($paymentMethods as $paymentMethod)
              <div class="dropdown-item custom-control custom-checkbox px-5">
                <input type="checkbox" class="custom-control-input" id="{{ $paymentMethod->slug }}"
                  wire:click="filterPaymentMethod('{{ $paymentMethod->id }}')"
                  {{ in_array($paymentMethod->id, $selectedPaymentMethod) ? 'checked' : '' }}>
                <label class="custom-control-label"
                  for="{{ $paymentMethod->slug }}">{{ $paymentMethod->nama }}</label>
              </div>
            @endforeach
          </div>
        </div>
      ...
      <ul class="list-group list-group-flush">
        @forelse ($userPayments as $payment)
          <li class="list-group-item d-flex justify-content-between align-items-center" style="cursor:pointer;"
            wire:click="transactionDetail({{ json_encode($payment->id) }})">
            <i class="bi bi-lightning-fill mr-1 pacific-blue" style="font-size: 20px"></i> Pembayaran Tagihan
            {{ \Str::limit($payment->tanggal_bayar->monthName, 3, '') }} {{ $payment->tanggal_bayar->year }}
            <span
              class="badge
							@switch($payment->status)
									...
									@case('expire')
										...
										@break
									@default
							@endswitch badge-pill ml-auto">{{ $payment->status }}</span>
          </li>
        @empty
          ...
          <div class="card">
            ...
            <div class="card-body">
              <dl class="row">
                ...
                <dt class="col-md-5">Nama Pelanggan PAMSIMAS</dt>
                  ...
                  <div class="row justify-content-between align-items-center">
                    ...
                    @if ($this->payment->status == 'pending')
                      <div class="col text-right">
                        <button class="btn usafa-blue" id="btnChangePaymentMethod"
                          wire:click.prevent="$emit('changePaymentMethodRequest')"><strong>ubah</strong></button>
                      </div>
                    @endif
                  </div>
                </dd>
                ...
                <dd class="col-md-7">
                  @switch($this->payment->status)
                    @case('success')
                      <span class="badge pill-badge badge-success p-1 mb-2">{{ $payment->status }}</span>
                      <div>
                        <a href="{{ route('upload-proof-of-payment', $this->payment->id) }}"
                    ...
                    @case('pending')
                      ...
                      <div class="mt-1">
                        @if ($transactionDetail->transaction_status == 'pending')
                          <a
                            href="{{ route('payment.confirm', ['payment_method' => $payment->paymentMethod->slug, 'payment' => $payment->id]) }}">Lanjutkan
                            pembayaran</a>
                        @else
                          <a href="{{ route('upload-proof-of-payment', $this->payment->id) }}"
                            class="btn btn-light">{{ $this->payment->bukti_bayar ? 'Lihat Struk Pembayaran' : 'Upload Struk Pembayaran' }}</a>
                        @endif
                      </div>
                    @break
                    @case('failed' || 'cancel')
