...
class GenerateAlgoritma
{
    public function randomingProcess($i)
    {
        ...
        $teach  = Teach::whereLog(0)->inRandomOrder()->first();
        ...
        $check_class_id = Schedule::where('rooms_id', '=', $room->id)
            ->where('days_id', '=', $day->id)
            ->where('times_id', '=', $time->id)
            ->where('type', '=', $type)
            ->first();
        ...
        {
            $time   = Time::where('id','!=',$time->id)->inRandomOrder()->first();
            $params = [
                'teachs_id'     => $teach->id,
                'days_id'       => $day->id,
                'times_id'      => $time->id,
        ...
        {
            $room   = Room::where('id','!=',$room->id)->inRandomOrder()->first();
            $params = [
                'teachs_id'     => $teach->id,
                'days_id'       => $day->id,
                'times_id'      => $time->id,
        ...
        {      
            $countt = Teach::whereLog(0)->count();
                
            if($countt == 0){                    
                return true;
            }      
            $insert = Schedule::create($params);
            Teach::whereId($params['teachs_id'])->update(['log' => 1]);
            return $insert;
        }
    }
    ...
    {
        // Schedule::truncate(); 
        // query builder laravel-> memotong seluruh tabel yang akan menghapus semua baris & mengatur ulang ID penambahan otomatis ke nol.
        $kromosom = 1;
        for ($i = 0; $i < $kromosom; $i++)
        {
            $values = [];
            for ($j = 0; $j < $count_teachs+100; $j++)
            {                
